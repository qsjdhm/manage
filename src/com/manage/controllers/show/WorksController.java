package com.manage.controllers.show;
...
public class WorksController {
	...
	public ModelAndView forwardPageAction(@PathVariable(value="page") Integer page)throws UnsupportedEncodingException{
		
		ModelAndView modelAndView = new ModelAndView();
		// 重定向，此方法可以改变地址栏url
		modelAndView.setViewName("redirect:/works/"+page+"/works");
		
		return modelAndView;
	}
	
	/*
	 * 功能：获取页数，并且处理works页面显示的内容后转向works页面
	 * 参数：page  页数
	 * 返回：返回works页面字符串
	 */
	@RequestMapping(value="/{page}/works", method = {RequestMethod.GET})
	public ModelAndView forwardDataManipulateAction(@PathVariable(value="page") Integer page) throws UnsupportedEncodingException{
		
		// 因为前台分页插件的索引是从0开始，所以加1
		page = page +1;
		// 1.根据分类获得此类型下的文章总个数
		int count = articleService.getArticleLength(2);
		// 2.根据分类和每页个数获取此分类下文章的总页数
		int pageCount = articleService.getArticlePageCount(2, 10);
		// 3.根据分类、页数、每页个数获取此分类下的文章列表
		List <TArticle> worksArticles = articleService.getArticle(2, page, 10);
		GenerateHtml generateHtml = new GenerateHtml();
		String worksArticleHtml = generateHtml.generateWorksMainHtml(worksArticles);
		...
		modelAndView.addObject("worksArticleHtml", worksArticleHtml);
		...
		modelAndView.setViewName("column/works");
