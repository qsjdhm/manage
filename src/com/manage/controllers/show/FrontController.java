...
public class FrontController {
	...
	public ModelAndView forwardPageAction(@PathVariable(value="page") Integer page){
		...
		// 重定向，此方法可以改变地址栏url
		modelAndView.setViewName("redirect:/front/"+page+"/front");
		
		return modelAndView;
	}
	
	/*
	 * 功能：获取页数，并且处理front页面显示的内容后转向front页面
	 * 参数：page  页数
	 * 返回：返回front页面字符串
	 */
	@RequestMapping(value="/{page}/front", method = {RequestMethod.GET})
	public ModelAndView forwardDataManipulateAction(@PathVariable(value="page") Integer page) throws Exception{
		...
		page = page +1;
		// 1.根据分类获得此类型下的文章总个数
		int count = articleService.getArticleLength(5);
		// 2.根据分类和每页个数获取此分类下文章的总页数
		int pageCount = articleService.getArticlePageCount(5, 10);
		// 3.根据分类、页数、每页个数获取此分类下的文章列表
		List <TArticle> frontArticles = articleService.getArticle(5, page, 10);
		GenerateHtml generateHtml = new GenerateHtml();
		String frontArticleHtml = generateHtml.generateHomeMainHtml(frontArticles);
		...
		modelAndView.addObject("frontArticleHtml", frontArticleHtml);
		...
		modelAndView.setViewName("column/front");
