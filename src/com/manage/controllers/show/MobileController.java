...
public class MobileController {
	...
	public ModelAndView forwardPageAction(@PathVariable(value="page") Integer page){
		...
		// 重定向，此方法可以改变地址栏url
		modelAndView.setViewName("redirect:/mobile/"+page+"/mobile");
		
		return modelAndView;
	}
	
	/*
	 * 功能：获取页数，并且处理mobile页面显示的内容后转向mobile页面
	 * 参数：page  页数
	 * 返回：返回mobile页面字符串
	 */
	@RequestMapping(value="/{page}/mobile", method = {RequestMethod.GET})
	public ModelAndView forwardDataManipulateAction(@PathVariable(value="page") Integer page) throws Exception{
		...
		page = page +1;
		// 1.根据分类获得此类型下的文章总个数
		int count = articleService.getArticleLength(7);
		// 2.根据分类和每页个数获取此分类下文章的总页数
		int pageCount = articleService.getArticlePageCount(7, 10);
		// 3.根据分类、页数、每页个数获取此分类下的文章列表
		List <TArticle> mobileArticles = articleService.getArticle(7, page, 10);
		...
		modelAndView.addObject("mobileArticleHtml", mobileArticleHtml);
		...
		modelAndView.setViewName("column/mobile");
