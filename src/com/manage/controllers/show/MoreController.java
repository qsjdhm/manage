...
public class MoreController {
	...
	public ModelAndView forwardPageAction(@PathVariable(value="page") Integer page){
		...
		// 重定向，此方法可以改变地址栏url
		modelAndView.setViewName("redirect:/more/"+page+"/more");
	 ...
	 * 返回：返回home页面字符串
	 */
	@RequestMapping(value="/{page}/more", method = {RequestMethod.GET})
	public ModelAndView forwardDataManipulateAction(@PathVariable(value="page") Integer page) throws Exception{
		...
		// 3.根据推荐数获取文章列表
		List <TArticle> mainArticles = articleService.getArticleByRecomNumber(0, page, 10);
		...
		modelAndView.setViewName("column/more");
