...
public class JavaController {
	...
	public ModelAndView forwardPageAction(@PathVariable(value="page") Integer page){
		...
		// 重定向，此方法可以改变地址栏url
		modelAndView.setViewName("redirect:/java/"+page+"/java");
		
		return modelAndView;
	}
	
	/*
	 * 功能：获取页数，并且处理java页面显示的内容后转向java页面
	 * 参数：page  页数
	 * 返回：返回java页面字符串
	 */
	@RequestMapping(value="/{page}/java", method = {RequestMethod.GET})
	public ModelAndView forwardDataManipulateAction(@PathVariable(value="page") Integer page) throws Exception{
		...
		page = page +1;
		// 1.根据分类获得此类型下的文章总个数
		int count = articleService.getArticleLength(6);
		// 2.根据分类和每页个数获取此分类下文章的总页数
		int pageCount = articleService.getArticlePageCount(6, 10);
		// 3.根据分类、页数、每页个数获取此分类下的文章列表
		List <TArticle> javaArticles = articleService.getArticle(6, page, 10);
		GenerateHtml generateHtml = new GenerateHtml();
		String javaArticleHtml = generateHtml.generateHomeMainHtml(javaArticles);
		...
		modelAndView.addObject("javaArticleHtml", javaArticleHtml);
		...
		modelAndView.setViewName("column/java");
